...
member_login_form = pyforms.Form(
    pyforms.Textbox("username", rules.len_of(1, 32), description=u"用户账号", size=32,required="required",**input_style),
...
def member_join_form(nodes=[]): 
    return pyforms.Form(
        ...
        pyforms.Textbox("realname", rules.len_of(2,32), description=u"用户ID(必填)", required="required",**input_style),
        pyforms.Dropdown("sex", description=u"♂&♀", args=sexopt.items(),required="required", **input_style),
        pyforms.Textbox("age", rules.is_number, description=u"年龄(必填)", size=3,required="required",**input_style),
        pyforms.Textbox("username", rules.is_alphanum3(6, 32), description=u"登录账号(必填)", size=32,required="required",**input_style),
        ...
        pyforms.Textbox("idcard", rules.len_of(0,32), description=u"序列码", **input_style),
        pyforms.Textbox("mobile", rules.len_of(0,32),description=u"QQ", **input_style),
        pyforms.Textbox("address", description=u"二次校验码",hr=True, **input_style),
...
password_mail_form =  pyforms.Form(
        pyforms.Textbox("member_name", rules.len_of(1, 64),description=u"请输入账号", required="required",**input_style),
        pyforms.Button("submit",  type="submit", html=u"<b>提交</b>", **button_style),
        title=u"重置密码请求",
